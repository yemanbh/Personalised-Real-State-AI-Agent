...
The "HomeMatch" project aims to develop a real estate recommendation application 
using Python, Large Language Models (LLMs), and vector databases. 
The application guides users through generating synthetic real estate listings using 
an LLM, which are then stored in a vector database (such as ChromaDB or LanceDB) for 
...
semantic similarity. The application also includes functionality to dynamically augment 
...
buyers' needs while ensuring factual integrity is maintained. The system is designed to be flexible,
 ...
 A notebook that shows how to run the real state agent is also provide.
 
 ## Synthetic Data Generation
 
 ### 1. Generate house description
 
 ### 1. Generate Generating Real Estate Listings with an LLM
...
the model to generate the data. Formats like CSV (Comma-Separated Values) or JSON 
...
### 1. Generate Generating Real Estate Listings Images with Vision Language Model
...
### Multimodal Embedding and Storing in a Vector Store
